...
function display_help() {
    cat << EOF
thumbnail-extract v0.0.17 extracts PNG thumbnails from a given video file 
...
do 
    if [ "$USE_TIMESTAMPS" = true ] ; then
        ...
        ffmpeg -ss "$TS" -i "$FILE" -frame_pts true -frames:v 1 -q:v 1 "$FILE""_thumb_"$i"_%d.$OUTPUT_FORMAT"
    else 
