...
function display_help() {
    cat << EOF
thumbnail-extract v0.0.16 extracts PNG thumbnails from a given video file 

Usage (items inside [] are optional arguments):

./thumbnail-extract.sh -f|--file <file> [-t|--timestamps hh:mm:ss,hh:mm:ss  -c|--count <number> ]
...
-c or --count       Number of images to extract from randomised timestamps. Cannot be used in combinationg with -t|--timestamps. If -t present it will take precedence.
...
do 
    if [ "$USE_TIMESTAMPS" = true ] ; then
        ...
        ffmpeg -ss "$TS" -i "$FILE" -frame_pts true -frames:v 1 -q:v 2 "$FILE""_thumb_"$i"_%d.png"
    else 
