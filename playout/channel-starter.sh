...
function display_help() {
    cat << EOF
channel-starter.sh v0.0.9 starts playout of a channel by reading the configuration in $BASE_LOCATION/<channel_name>-config.
...
IPTV_LIVE_PIDS_LOCATION="$BASE_LOCATION/iptv-live-$CHANNEL_IDENTIFIER.pids"
...
for KEY in "${!STREAMS[@]}"; do
    PORT=${STREAMS[$KEY]}
    mapfile -t FFMPEG_PIDS < <( pgrep -d " " -f ^ffmpeg.*$CHANNEL_IDENTIFIER.*$PORT.* )
...
for KEY in "${!STREAMS[@]}"; do
    PORT=${STREAMS[$KEY]}
    /usr/local/bin/iptv-live.sh --m3u /home/edu/playlists/$CHANNEL_IDENTIFIER\_$KEY/${MONTH,,}-$DAY.m3u --url 127.0.0.1:$PORT >> $BASE_LOCATION/$TODAY-$CHANNEL_IDENTIFIER-playout.log & 
    PIDS+=($!)
done 
